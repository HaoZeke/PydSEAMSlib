...
def test_templates_assign_fcc_and_bcc():
    fcc = _fcc_cloud()
    bcc = _bcc_cloud()
    fcc_n = yoda.neighListO(rcutoff=3.2, yCloud=fcc, typeI=1)
    bcc_n = yoda.neighListO(rcutoff=4.0, yCloud=bcc, typeI=1)

    fcc_hits = yoda.classifyTemplates(fcc, fcc_n, 12)
    bcc_hits = yoda.classifyTemplates(bcc, bcc_n, 8)
...
def test_soap_spectrum_size_and_cosine():
    cloud = _fcc_cloud()
    n_list = yoda.neighListO(rcutoff=3.2, yCloud=cloud, typeI=1)
    a = list(yoda.soapSpectrum(cloud, 0, n_list, 3, 6, 3.2))
    b = list(yoda.soapSpectrum(cloud, 1, n_list, 3, 6, 3.2))
...
def test_linear_classifier_separates_fcc_bcc_voronoi():
    ...
    def pack(cloud, cut):
        q4 = yoda.steinhardtQlVoronoi(cloud, cut, 4)
        q6 = yoda.steinhardtQlVoronoi(cloud, cut, 6)
        q8 = yoda.steinhardtQlVoronoi(cloud, cut, 8)
