...
``hbond``) needs an H cloud. Pass ASE ``Atoms`` that include ``H``,
a LAMMPS dump with hydrogen, or set ``bonded="cutoff"``.
...
``.xyz`` needs ``yoda.readXYZ``. Check
``available_readers()["xyz"]``.
